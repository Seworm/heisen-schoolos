...
export default async function StudentImportPage() {
  ...
  return <section><h1 className="text-3xl font-semibold">Student import center</h1><p className="mt-1 text-sm text-slate-500">Upload, validate, and review school-scoped CSV imports.</p><form action={importStudentsAction} encType="multipart/form-data" className="mt-6 rounded-xl border bg-white p-6"><input type="file" name="file" accept=".csv,text/csv" required /><button className="ml-4 rounded bg-slate-900 px-4 py-2 text-white">Validate and import</button><p className="mt-3 text-xs text-slate-500">Required columns: student_number, first_name, last_name, gender.</p></form><div className="mt-6 rounded-xl border bg-white"><h2 className="border-b p-4 font-semibold">Import history</h2>{history.map((item) => <div key={item.id} className="flex justify-between border-b p-4 text-sm"><span>{item.filename} · {item.validRows}/{item.totalRows} valid</span><span className="text-slate-500">{item.status}</span></div>)}{!history.length && <p className="p-6 text-sm text-slate-500">No imports yet.</p>}</div></section>;
}
