...
import { deleteCalendarEvent } from "../operations/actions";
...
export default async function CalendarPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8">
    ...
    <CalendarForms />
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-semibold">Upcoming events</h2><div className="mt-4 divide-y divide-slate-100">{events.filter((event) => event.endsAt >= new Date()).map((event) => <div key={event.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="font-medium">{event.title}</p><p className="text-sm text-slate-500">{event.type} · {event.startsAt.toLocaleString()}</p>{event.description && <p className="mt-1 text-xs text-slate-400">{event.description}</p>}</div><div className="flex items-center gap-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{event.allDay ? "All day" : "Scheduled"}</span><form action={deleteCalendarEvent}><input type="hidden" name="id" value={event.id} /><button className="text-xs font-medium text-rose-600 hover:underline">Cancel</button></form></div></div>)}{events.filter((event) => event.endsAt >= new Date()).length === 0 && <p className="py-6 text-sm text-slate-500">No upcoming events.</p>}</div></section>
